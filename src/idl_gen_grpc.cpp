...
public:
  ...
  FlatBufMethod(const RPCCall *method) : method_(method) {
    ...
    if (val) {
      if (val->constant == "client")
        streaming_ = kClient;
      if (val->constant == "server")
        streaming_ = kServer;
      if (val->constant == "bidi")
        streaming_ = kBiDi;
    }
  }

  grpc::string GetLeadingComments(const grpc::string) const override {
    return "";
  }

  grpc::string GetTrailingComments(const grpc::string) const override {
    return "";
  }

  std::vector<grpc::string> GetAllComments() const override {
  ...
  std::vector<std::string> get_input_namespace_parts() const override {
    return (*method_->request).defined_namespace->components;
  }

  std::string get_input_type_name() const override {
    return (*method_->request).name;
  }

  std::vector<std::string> get_output_namespace_parts() const override {
    return (*method_->response).defined_namespace->components;
  }

  std::string get_output_type_name() const override {
  ...
  bool get_module_and_message_path_input(
      ...
      grpc::string /*import_prefix*/) const override {
    return true;
  }
  ...
  std::string get_fb_builder() const override { return "builder"; }

  std::string input_type_name() const override {
    return GRPCType(*method_->request);
  }

  std::string output_type_name() const override {
    return GRPCType(*method_->response);
  }

  bool NoStreaming() const override { return streaming_ == kNone; }

  bool ClientStreaming() const override { return streaming_ == kClient; }

  bool ServerStreaming() const override { return streaming_ == kServer; }

  bool BidiStreaming() const override { return streaming_ == kBiDi; }
...
public:
  FlatBufPrinter(std::string *str, const char indentation_type)
      : str_(str), escape_char_('$'), indent_(0), indentation_size_(2),
        indentation_type_(indentation_type) {}

  void Print(const std::map<std::string, std::string> &vars,
             const char *string_template) override {
  ...
  void SetIndentationSize(const size_t size) override {
    FLATBUFFERS_ASSERT(str_->empty());
    indentation_size_ = size;
  }

  void Indent() override { indent_++; }
...
public:
  ...
  std::string filename() const override { return file_name_; }

  std::string filename_without_ext() const override {
    return StripExtension(file_name_);
  }

  std::string message_header_ext() const override {
    return parser_.opts.filename_suffix + ".h";
  }

  std::string service_header_ext() const override { return ".grpc.fb.h"; }
  ...
  std::string additional_headers() const override {
    switch (language_) {
    case kLanguageCpp: {
  ...
  int service_count() const override {
  ...
  std::unique_ptr<grpc_generator::Printer>
  CreatePrinter(std::string *str,
                const char indentation_type = ' ') const override {
...
public:
  ...
  bool generate() override {
    code_.Clear();
    code_ += "# Generated by the gRPC Python protocol compiler plugin. "
             "DO NOT EDIT!\n";
    code_ += "import grpc\n";
  ...
  std::string GenerateFileName() {
    ...
    if (!namespace_dir.empty())
      grpc_py_filename += kPathSeparator;
...
public:
  ...
  static std::string GeneratedFileName(const std::string &path,
    ...
    if (is_interface)
      return path + file_name + "_grpc.d.ts";
