...
private:
  ...
  std::string GenTypeBasic(const Type &type, bool enableLangOverrides) const {
    ...
    if (enableLangOverrides) {
      if (IsEnum(type))
        return NamespacedName(*type.enum_def);
  ...
  void GenEnum_ObjectAPI(EnumDef &enum_def, std::string *code_ptr,
    ...
    if (opts.cs_gen_json_serializer) {
      ...
      code += "      foreach (var _o in _olist) { this.WriteJson(writer, _o, "
              "serializer); }\n";
      ...
      code += "        reader.Read();\n";
      code += "        _olist[_j] = this.ReadJson(reader, _olist[_j], "
              "serializer);\n";
      code += "      }\n";
      code += "      reader.Read();\n";
      code += "      return _olist;\n";
      ...
      case BASE_TYPE_VECTOR: {
        if (field_has_create.find(&field) != field_has_create.end()) {
          ...
          case BASE_TYPE_STRING: {
            ...
            to_array +=
                "builder." + create_string + "(_o." + property_name + "[_j])";
            break;
          }
          case BASE_TYPE_STRUCT:
  ...
  void GenStruct_ObjectAPI(StructDef &struct_def, std::string *code_ptr,
        ...
        if (IsUnion(field.value.type)) {
          ...
          if (IsVector(field.value.type)) {
            ...
            code += "      var _o = new " + utype_name + "[this." + camel_name +
                    ".Count];\n";
            code += "      for (var _j = 0; _j < _o.Length; ++_j) { _o[_j] = "
                    "this." +
                    camel_name + "[_j].Type; }\n";
            code += "      return _o;\n";
            code += "    }\n";
    ...
    if (opts.cs_gen_json_serializer &&
      ...
      code += "    return Newtonsoft.Json.JsonConvert.SerializeObject(this, "
